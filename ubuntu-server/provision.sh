...
Install security packages and remove telemetry on Ubuntu Server.

Options:
  --keep-snap         Do not remove snapd (keep if you use snap packages)
...
#
# fail2ban            Monitors log files and bans IPs with repeated auth failures.
...
#                     evident log. Required by many compliance frameworks.
#
...
#
# unattended-upgrades Automatically installs security updates. Critical for any
#                     server — manual patching cadence is too slow for security fixes.
#
...
#
# apport              Ubuntu crash reporter. Captures crash data (including
#                     potentially sensitive memory contents) and offers to send
#                     it to Canonical's servers. Not useful on a hardened server.
#
...
#                     for snap metrics. Also creates loopback devices for each
#                     snap that clutter 'df' output and some monitoring tools.
#                     Kept by default if --keep-snap is passed.
# ---------------------------------------------------------------------------
...
remove_telemetry_packages() {
    ...
    if [[ "${KEEP_SNAP}" -eq 0 ]]; then
        packages_to_remove+=(snapd)
    else
        warn "Keeping snapd (--keep-snap)"
    fi
...
_cleanup_snap_leftovers() {
    ...
    # (e.g., by gnome-software or ubuntu-desktop)
...
main() {
    ...
    log "=== secure_oss Ubuntu Server provision.sh v${SCRIPT_VERSION} ==="
    ...
    log "  Options: --ssh-port PORT  --skip SECTIONS  --dry-run"
    echo ""
}
